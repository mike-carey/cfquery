...
func (s *OrgService) GetStorage() (map[string]cfclient.Org, error) {
	_, err := s.GetAll()
	if err != nil {
		return nil, err
	}
...
func (s *OrgService) GetManyByGuid(guids ...string) (map[string]cfclient.Org, error) {
	pool := make(map[string]cfclient.Org, len(guids))
...
func (s *OrgService) GetAll() ([]cfclient.Org, error) {
	s.lock()

	if s.filled {
		logger.Infof("Reusing storage")
		siSlice := make([]cfclient.Org, 0, len(s.storage))
	...
	go func(s *OrgService, sis []cfclient.Org) {
